...
class SMMRPY:
    ...
    async def get_smmry(self, url: str, **kwargs):
        ...
        IncorrectSubmissionVariables
            ...
            passed.
        IntentionalRestriction
            Intentional restriction. Could be
        ...
        if breaks:
            params['SM_WITH_BREAK'] = 'breaks'
        if not quotes:
            params['SM_QUOTE_AVOID'] = 'avoid'
        ...
        if error_code:
            if error_code == 0:
                ...
                raise IntentionalRestriction(message)
            elif error_code == 3:
                raise SummarizationError(message)
            else:
                raise Exception('We failed and don\'t know why :(')
